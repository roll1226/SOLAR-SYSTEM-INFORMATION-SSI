...
window.addEventListener('load', function () {
  const neptunePlanet = new planet('../images/neptune.jpg', 50, 20, 20);
  const { scene, camera, controls, renderer } = createPlanetScene();

  const neptuneMesh = new THREE.Mesh(neptunePlanet.Geometry, neptunePlanet.Material);
...
new Vue({
  el: '#maintitle',
  data: { maintitle: '<h1>海王星/NEPTUNE</h1>' }
});
